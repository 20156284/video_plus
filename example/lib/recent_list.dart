...
class SamplesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const VideoAppBar(title: 'Online Samples'),
      body: ListView.builder(
          itemCount: samples.length,
          itemBuilder: (context, index) {
            final mediaUrl = samples[index];
...
class _RecentMediaListState extends State<RecentMediaList> {
  ...
  onHistoryChanged(int v) {
    final count = prefs.getInt('recent_count', defaultValue: 0).getValue();
    final newest = prefs.getInt('recent_newest', defaultValue: 0).getValue();
...
Future<void> addToHistory(MediaUrl mediaUrl) async {
  ...
  var newest = prefs.getInt('recent_newest', defaultValue: 0).getValue();
  var count = prefs.getInt('recent_count', defaultValue: 0).getValue();
