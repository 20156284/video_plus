...
@implementation VideoPlusPlayer {
    ...
    VideoPlusHostOption *_hostOption;
    int _state;
    int _pid;
    int64_t _vid;
}
...
- (instancetype)initWithRegistrar:(id<FlutterPluginRegistrar>)registrar {
    ...
    if (self) {
        ...
        _hostOption = [[VideoPlusHostOption alloc] init];
        _lastBuffer = nil;
        if (renderType == 0) {
...
- (void)onStateChangedWithNew:(int)newState andOld:(int)oldState {
    VideoPlusPlugin *plugin = [VideoPlusPlugin singleInstance];
    if (plugin == nil)
        return;
